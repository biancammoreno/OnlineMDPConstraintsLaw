...
class MirrorDescentSet2:
    """
    Class to compute Online Mirror Descent with changing transition costs
    """

    def __init__(self, env, online, n_agents=100, lr= 0.01, reward_type='entropy_max'):
        self.env = env
        self.lr = lr
    ...
    def bonus_state_action_value(self, mu, policy):
        ...
        for n in range(self.N_steps - 1, 0, -1):
            for x in range(self.S):
                for a in range(self.A):
                    Q[n-1,x,a] = reward[n-1,x] + self.C * (1) * (self.N_steps - n)/np.maximum(np.sqrt(self.n_counts[x,a]),1) # I deleted the lambda here
                    for x_next in range(self.S):
                        Q[n-1,x,a] += self.P[x,a,x_next] * np.dot(policy[n, x_next,:], Q[n,x_next,:])
    ...
    def iteration(self, n_iterations):
        ...
        if self.count_step == 0:
            self.error = []
            # initialize mu with respect to the uniform policy (initial policy) and with mu_0
            # for n in range(self.N_steps):
                # self.mu[n,:] = self.nu0
            self.mu = self.mu_induced(self.policy, self.P, self.nu0)
            self.gap = 10
            self.init_dist = self.nu0 # stores the true initial distribution
            self.estimate_init_dist = self.nu0 # stores the estimated initial distribution
            self.rho_diff = []
            self.rho_diff_estimate = []
        ...
        for iter in range(n_iterations):
            ...
            # 3) Update the state-action distribution (the initial dist. equal to the estimate of the final one), or the offline case 
            if self.online == True:
                mu_lamb = self.mu_induced(policy_lamb, self.P, self.estimate_init_dist)
            else:
            ...
            # once the constraint is satisfied we give the final updates
            ...
            # 6) Update the probability transitions if needed (online case) - agents start at rho_t
            if self.online == True:
                self.P, self.mu_empirical = self.estimate_transition(self.init_dist, self.n_agents)
            # 7) Estimate the next initial distribution 
            if self.online == True:
                # first update the true next initial distribution
                true_mu = self.mu_induced(self.policy, self.true_P, self.init_dist) # true next initial distribution
                self.init_dist = true_mu[-1,:]
                # update the probability transition to estimate the initial distribution
                self.P_tilde, _ = self.estimate_transition(self.estimate_init_dist, int(self.n_agents * 0.1))
                # update the estimate of the initial distribution
                estimate_mu = self.mu_induced(self.policy, self.P_tilde, self.estimate_init_dist)
                self.estimate_init_dist = estimate_mu[-1,:]
            else:
                self.init_dist = self.mu[-1,:]
            ...
            # L1 difference between true initial distribution and the estimated one
            self.rho_diff_estimate.append(LA.norm(self.init_dist - self.estimate_init_dist, 1))
        

    def sample_policy(self, n, state):
        return np.random.choice(self.A, p=self.policy[n, state,:])

    def estimate_transition(self, initial_dist, n_agents):
        """
        Estimate transitions from n_agents playing the current policy starting at initial_dist
        """
        n_steps = n_agents * self.env.max_steps

        P = np.zeros((self.S, self.A, self.S))
        mu_empirical = np.zeros((self.N_steps, self.S))

        observation = self.env.reset(initial_dist)
        ...
        mu_empirical = mu_empirical/n_agents
